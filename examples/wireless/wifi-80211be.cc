...
// This example is used to validate 802.11be MIMO.
//
// It outputs plots of the throughput versus the distance
// for every EHT MCS value and from 1 to 4 MIMO streams.
//
...
// some 802.11be parameters (frequency, channel width and guard interval).
//
...
{
    std::ofstream file("80211be-throughput.plt");

    std::vector<std::string> modes = {
        "EhtMcs0",  "EhtMcs1",  "EhtMcs2",  "EhtMcs3",  "EhtMcs4",  "EhtMcs5",  "EhtMcs6",
        "EhtMcs7",  "EhtMcs8",  "EhtMcs9",  "EhtMcs10", "EhtMcs11", "EhtMcs12", "EhtMcs13",
    };

    bool udp{true};
    Time simulationTime{"5s"};
    double frequency{6.0}; // whether 2.4, 5.0 or 6.0 GHz
    ...
    cmd.AddValue(
        "frequency",
        "Whether working in the 2.4, 5.0 or 6.0 GHz band (other values gets rejected)",
        frequency);
    cmd.AddValue("udp", "UDP if set to 1, TCP otherwise", udp);
    cmd.Parse(argc, argv);

    Gnuplot plot = Gnuplot("80211be-throughput.eps");
    ...
    for (uint32_t i = 0; i < modes.size(); i++) // MCS
    {
        ...
        for (double d = 0; d <= 100;) // distance
        {
            ...
            uint8_t nStreams = 1 + (i / 4); // number of MIMO streams
            ...
            phy.Set("ChannelSettings",
                    StringValue(std::string("{0, ") + (channelBonding ? "40, " : "20, ") +
                                (frequency == 2.4
                                     ? "BAND_2_4GHZ"
                                     : (frequency == 6.0 ? "BAND_6GHZ" : "BAND_5GHZ")) +
                                ", 0}"));

            WifiMacHelper mac;
            WifiHelper wifi;
            if (frequency == 6.0 || frequency == 5.0)
            {
            ...
            Config::Set(
                ...
                TimeValue(NanoSeconds(shortGuardInterval ? 800 : 1600)));
            ...
            const auto maxLoad = EhtPhy::GetDataRate(i,
                                                    channelBonding ? MHz_u{40} : MHz_u{20},
                                                    NanoSeconds(shortGuardInterval ? 800 : 1600),
                                                    nStreams);
